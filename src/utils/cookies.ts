...
export function encryptCookie(user: User): string {
    ...
    const cipher = createCipheriv(ALGO, process.env.SECRET_KEY, process.env.SECRET_IV);
    return cipher.update(JSON.stringify(payload), "utf-8", "base64") + cipher.final("base64");
}

export function decryptCookie(cookie: string): User | null {
    const decipher = createDecipheriv(ALGO, process.env.SECRET_KEY, process.env.SECRET_IV);
    const decrypted = decipher.update(cookie, "base64", "utf-8") + decipher.final("utf-8");
